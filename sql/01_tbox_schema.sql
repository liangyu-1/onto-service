...
CREATE TABLE IF NOT EXISTS ontology_property (
    ...
    is_measure BOOLEAN COMMENT '是否为聚合指标 / measure',
    semantic_role VARCHAR(64) COMMENT '语义角色：identifier / dimension / state / metric',
    semantic_aliases ARRAY<VARCHAR(256)> COMMENT '属性别名数组，辅助 LLM grounding',
    hidden BOOLEAN DEFAULT FALSE COMMENT '是否默认隐藏，适用于敏感字段或内部字段',
    description TEXT COMMENT '面向人类的简短说明',
...
CREATE TABLE IF NOT EXISTS ontology_relationship (
    ...
    outgoing_is_multi BOOLEAN COMMENT 'TRUE 表示 source.outgoing_name 类型为 MULTIROW<target_label>',
    incoming_is_multi BOOLEAN COMMENT 'TRUE 表示 target.incoming_name 类型为 MULTIROW<source_label>',
...
CREATE TABLE IF NOT EXISTS ontology_logic (
    ...
    deterministic BOOLEAN COMMENT '是否确定性；影响是否可用于 precondition',
...
CREATE TABLE IF NOT EXISTS ontology_logic_execution_binding (
    ...
    enabled BOOLEAN COMMENT '该绑定是否可被本体层使用',
